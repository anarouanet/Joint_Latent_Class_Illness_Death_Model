...
!                           if(CurrentProcessorID.eq.0) write(*,*)(varM(m,m1),m1=1,np)                     
                        ...
                        ier=0
                        det=0.d0
                        CALL DSINV(VarV,np,eps,ier,det)   
                        if (ier.eq.-1) then
...
!                           if(CurrentProcessorID.eq.0) write(*,*)'    ',(varMg(m,m1),m1=1,np*ng)                 
                        ...
                        ier=0
                        det=0.d0
                        CALL DSINV(VarVg,np*ng,eps,ier,det)   
                        if (ier.eq.-1) then
                           if(CurrentProcessorID.eq.0) write(*,*)'dsinv does not work for Wald test'
                           stop
                        end if
...
!                           if(CurrentProcessorID.eq.0) write(*,*)'    ',(varM(m,m1),m1=1,ng)                     
                        ...
                        ier=0
                        det=0.d0
                        CALL DSINV(VarV,ng,eps,ier,det)   
                        if (ier.eq.-1) then
                           if(CurrentProcessorID.eq.0) write(*,*)'dsinv does not work for Wald test'
                           stop
                        end if
...
!c     Vi en vecteur
	var_uYT=0.d0
         ...
         CALL DSINV(var_uYT,nmestot,eps,ier,det)
...
! {{{
      ...
      double precision function funcpa(b,npm,id,thi,jd,thj)
        ...
        implicit none
        integer::npm,ier,nnn,ntrtot,nrisq_curr,numact,numprob
...
!variables explicatives pour i
		...
		do k=1,nmes(i,q)
		    ...
		    jjj=jjj+1
			    ...
			    Z(jj,jjj+1)=temp!((b1((nvarprob)*(ng-1)+nrisq+nvarxev

!t+1)-65.0d0)/10.d0-t(Navant_i(i)+jj))**(k-1)*temp
		            If (k.gt.2) then
	      ...
	      jj=0
                 ...
                 Else if (idglob(k).eq.2.and.idg(k).eq.0) then
                    ...
                    Do q=1,nq
                       Do j=1,nmes(i,q)
                          ...
                          Do m=0,np
   			     temp=(b1((nvarprob)*(ng-1)+nrisq+nvarxevt+1)-65.0d0)/10.d0
                             !C Attention : changer les Xglob avec interaction
                             if (m.ge.1) then
                                XglobssG(jj,l+m)=X(i,k)*(t(Navant_i(i)+jj)-temp)!((t(Navant_i(i)+jj)-temp)**m)*(0.1**(m-1))
			        temp=dble(t(Navant_i(i)+jj)-(b1((nvarprob)*(ng-1)+nrisq+nvarxevt+1)-65.0d0)/10.d0)
              ...
              End do
                 ...
                 IF (QUECONT.eq.1) then
                    ...
                    ier=0
                    CALL DSINV(Vi,nmestot,eps,ier,det)
                    if (ier.eq.-1) then
                       !  if(CurrentProcessorID.eq.0) write(*,*)'dsinv vi ier',ier,' i=',i,' nmes(i)=',nmestot
                       !  if(CurrentProcessorID.eq.0) write(*,*)'b1', (b1(k),k=1,npm)
...
!c     Vi en vecteur
         ...
         ier=0
         CALL DSINV(Vi,nmoinsc,eps,ier,det)
         if (ier.eq.-1) then
...
!               end if
           ...
           !C------ creation de la matrice des puissances du temps -----
		    ...
		    jjj=jjj+1
                    ...
                    Do k=2,np+1
			    Z(jj,jjj)=(t(Navant_i(i)+jj)-(b1((nvarprob)*(ng-1)+nrisq+nvarxevt+g)-65.0d0)/10.d0)
			    temp=dble(t(Navant_i(i)+jj)-(b1((nvarprob)*(ng-1)+nrisq+nvarxevt+g)-65.0d0)/10.d0)
			    !Z(jj,jjj+1)=(t(Navant_i(i)+jj)-(b1((nvarprob)*(ng-1)+nrisq+nvarxevt+g)-65.0d0)/10.d0)**(k-1)*temp
			    temp=(temp**2+trn)**0.5d0
			    Z(jj,jjj+1)=temp!
...
! {{{
        ...
        subroutine vraisSmooth(i,nlenb,b,bvexp,g,vexpi,tacti,valfin,tronc,score_gTT,score_gYT,SUTi0,STi0,postfit,trans)
!	STi0= exp(-A01)
        use commun
...
!  double precision,intent(in)::thi,thj
        integer,intent(in)::i,nlenb,postfit,trans
	...
	DO i=1,ns
	    ...
	    temp=0.d0
		  ...
		  temp=ppi(i,g)
            ...
            END DO
		write(9899,*) mmm,X(i,1),X(i,2),ta(i),tb(i),tc(i),td(i),X(i,3),X(i,4)!,ppi(i,:)
	end do
