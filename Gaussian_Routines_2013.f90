...
! ===================== GAUSSHERMITE NODES =======================
      ...
      subroutine gaussleg(gauss,npg)

!
!C
!C     Gauss-Legendre pour integrale sur -1,1
        !npg=nb de points : =15,20,30,40,50 uniquement
!C
      ...
      integer :: npg,i
      DATA ( Wg(I,15), Tg(I,15), I = 1,8) / &
	0.2025782419255613,	0.0000000000000000,&
...
!ccccccccccccccccccccccccccccccccccccccccccccccc
         ...
         DO I = 1, int(NPG/2)
            GAUSS(1,I) = -Tg(int((NPG+1)/2)+1-I,NPG)
            GAUSS(2,I) =  Wg(int((NPG+1)/2)+1-I,NPG)
            GAUSS(1,int((NPG+1)/2)+I) = Tg(MOD( NPG, 2 )+I,NPG)
            GAUSS(2,int((NPG+1)/2)+I) = Wg(MOD( NPG, 2 )+I,NPG)
         END DO

      IF ( MOD( NPG, 2 ) .EQ.1) THEN
         GAUSS(1, int(NPG/2) + 1 ) = 0.D0
         GAUSS(2, int(NPG/2) + 1 ) = Wg(1,NPG )
      END IF
